...
const docsCommit = "2b58caa8d60147df494ce995f3777944a400b9a9";
...
async function main() {
  ...
  const verifyInvocation = resolveNpxInvocation([
    ...
    "--receipt", repoPath(join(output, "root-receipt.json")),
    "-j",
  ]);
  ...
  await writeFile(join(output, "receipt-proof.json"), `${JSON.stringify(proof, null, 2)}\n`);
  ...
  process.stdout.write(`RECEIPT_PASS ${proof.receipt_ref}\n`);
}
