...
test("validation skill declares the complete governed network contract", async () => {
  ...
  assert.match(xYaml, /profile:\s*unrestricted-local-dev/);
  assert.match(xYaml, /network:\s*true/);
  assert.match(xYaml, /require_enforcement:\s*false/);
...
test("Linux workflow uses the supported explicitly approved direct sandbox and masks the private seed", async () => {
  const workflow = await text(
    new URL(".github/workflows/validate-sourcey-adoption.yml", root),
  );

  const provisionIndex = workflow.indexOf("name: Prepare governed validation directories");
  ...
  assert.match(provisionStep, /set -euo pipefail/);
  assert.match(provisionStep, /mkdir -p "\$runx_receipt_dir" "\$validation_output_dir"/);
  assert.match(provisionStep, /profile=unrestricted-local-dev/);
  assert.match(provisionStep, /network=true/);
  assert.match(provisionStep, /require_enforcement=false/);
  assert.match(provisionStep, /operator_approval=required/);
  assert.match(provisionStep, /runtime_enforcer=direct/);
  assert.match(provisionStep, /sandbox-policy\.txt/);
  assert.doesNotMatch(workflow, /apt-get|command -v bwrap|bwrap --version/);
  ...
  assert.doesNotMatch(uploadPaths, /bubblewrap/);
});
